        ...
        try {
          const sensorsPromises = stationsData.map(async (station) => {
            const sensorsResponse = await axios.get(
              `https://pogoda-d9om.onrender.com/https://api.gios.gov.pl/pjp-api/rest/station/sensors/${station.id}`, 
              {
            ...
            for (const sensor of sensorsData) {
              try {
                const measurementsResponse = await axios.get(
                  `https://pogoda-d9om.onrender.com/https://api.gios.gov.pl/pjp-api/rest/data/getData/${sensor.id}`, 
                  {
                    headers: { 'X-Requested-With': 'XMLHttpRequest' }
                  }
                );
